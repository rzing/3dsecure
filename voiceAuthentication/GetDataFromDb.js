	...
	console.log("Input Details are:" + phrase_input);
 	...
 	var param = {
  		...
  		if (err) {
   			...
   			console.log("Dynamo DB zipcode value is: "+data.Items[0].phrase);	
				...
				callback(null, buildResponse(true,name,phrase,customerId,message))
   			} 
			else {
...
function buildResponse(isSuccess,name,phrase,customerId,message) {
 	if (isSuccess) {
  		return { 
			name: name,
			phrase: phrase,
